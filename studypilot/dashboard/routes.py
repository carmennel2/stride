...
Aggregations are computed in Python over the user's sessions/tasks. Data
volumes per user are small enough that this is simpler than wiring SQL
GROUP BYs and survives the SQLite-vs-postgres-vs-anything portability
concern. If a user ever has 10k+ sessions, swap to SQL.
"""
...
from studypilot.models import StudySession, Subject, Task

bp = Blueprint("dashboard", __name__, url_prefix="/dashboard")

# Day-of-week labels in Python's Monday=0 order so the bar chart x-axis
# is stable and doesn't depend on the user's locale.
...
def _kpis(user_id: int) -> dict:
    """Compute the four headline numbers shown at the top of the dashboard."""
    today = date.today()
...
def _by_subject(user_id: int) -> tuple[list[str], list[float], list[str]]:
    """Hours studied per subject. Returns (labels, hours, colors)."""
    ...
    for s in sessions:
        # We need the subject_id via the task — one extra attribute access.
...
def _by_weekday(user_id: int) -> list[float]:
    """Hours studied broken down by day-of-week (Mon .. Sun)."""
    sessions = StudySession.query.filter_by(user_id=user_id).all()
    minutes = [0] * 7
    for s in sessions:
...
def _due_soon(user_id: int, days: int = 7) -> list[Task]:
    """Pending/in-progress tasks due within the next `days` days."""
    today = date.today()
...
@login_required
def dashboard():
    """Main dashboard view — KPIs, two charts, and the due-soon list."""
    uid = current_user.id
    kpis = _kpis(uid)
