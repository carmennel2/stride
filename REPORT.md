...
In addition to the username/password flow, Stride supports federated sign-in through Google. The integration is implemented with the Authlib library [11] using the OpenID Connect protocol: a single `_userinfo_from_google` function normalises Google's ID-token claims into a `(provider_user_id, email, name)` triple, and a `_login_with` helper handles user lookup, account linking, and creation. The abstraction layer is deliberately provider-agnostic — adding a second provider would require a new entry in `PROVIDERS`, a corresponding `_userinfo_from_<provider>` function, and a registration block in the application factory; no schema changes.

The data model follows the established convention for federated identity: a separate `oauth_identities` table maps the composite key `(provider, provider_user_id)` to a row in `users`, allowing one local account to carry multiple external identities should additional providers be added later. Brand-new identities — those whose email is not already known — create a fresh `User` with `password_hash = NULL`, denoting an OAuth-only account; the `User.check_password` method short-circuits to `False` for such accounts rather than crashing on the null hash. An existing email-account match links the Google identity onto the existing user rather than producing a duplicate.

The configuration model is deliberately permissive: Google's credentials live in environment variables (`GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`), and the application registers the provider with the OAuth client *only* if both halves of the credential pair are present. A context processor exposes the configured-provider list to templates, which iterate over it to decide which sign-in buttons to render. The OAuth route flashes a friendly "Sign in with Google isn't available right now" message and redirects to the form-based login if the credentials are missing, so a clicked button never lands on a 404. Username generation for new OAuth accounts sanitises the display name into a URL-safe candidate, falls back to the email's local part when no display name is available, and disambiguates collisions with existing accounts by appending a numeric suffix.
...
The suite runs in under ten seconds on a development laptop and is invoked with `python -m pytest tests/`. The test cases include direct verification of cross-user isolation (the authorisation contract), salt randomness on password hashing, the bias/variance behaviour of the regression, and every form's validation rules. Where a test must override the model loader to exercise a sanity guard, monkeypatching at the module level keeps the test self-contained.

### 10.2 Static analysis

`ruff check` runs clean across the whole codebase. The lint pass caught a small handful of unused imports during normal development — the kind of detritus that accumulates as a project grows — and the project's `pyproject.toml` keeps the rule set conservative but consistent. Type hints are used pervasively, though not enforced via static type-checking; the dynamic nature of Flask's request and SQLAlchemy's query objects makes a strict type-check noisier than the codebase's annotation density currently warrants.

### 10.3 Continuous integration

The project doesn't ship a CI configuration. The test invocation is CI-ready — `pip install -r requirements-dev.txt && python -m pytest tests/` is everything a workflow would need — but adding a YAML file felt like cargo for a single-author project. A real team would add it.
...
- **Continuous-integration pipeline.** A GitHub Actions workflow running install / lint / test on every push is a few lines of YAML and would catch regressions before they reach `main`.
